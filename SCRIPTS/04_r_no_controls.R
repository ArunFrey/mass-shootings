# ---
# SUPPLEMENTARY ANALYSIS: NO CONTROLS
# ---
...
m1 <- lm_robust(formula(model), 
                se_type = "stata", fixed_effects = ~ as.factor(year), 
                data = gss_analysis, subset = (treat_14==1|control==1))

m2 <- lm_robust(formula(paste(model, "+ treat_14 * party_cat")), 
                se_type = "stata", fixed_effects = ~ as.factor(year), 
                data = gss_analysis, subset = (treat_14==1|control==1))

# BY SCHOOL SHOOTING ------------------------

m1_noschool <- lm_robust(formula(paste(model)), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                         subset = (treat_14==1 & school_shooting==0|control==1))


m2_noschool <- lm_robust(formula(paste(model, "+ treat_14 * party_cat")), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                         subset = (treat_14==1 & school_shooting==0|control==1))

m1_school <- lm_robust(formula(paste(model)), 
                       se_type = "stata", fixed_effects = ~ as.factor(year), 
                       data = gss_analysis, 
                       subset = (treat_14==1 & school_shooting==1|control==1 & year_school==1))

m2_school <- lm_robust(formula(paste(model, "+ treat_14 * party_cat")), 
                       se_type = "stata", fixed_effects = ~ as.factor(year), 
                       data = gss_analysis, 
                       subset = (treat_14==1 & school_shooting==1|control==1 & year_school==1))
