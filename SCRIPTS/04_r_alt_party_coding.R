...
# Use alternative coding of "Independent" category, differentiating between
# Independents and other party affiliations
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_alt_party_coding/"
models_path <- "OUTPUT/MODELS/supp/r_alt_party_coding/"

# Specify model
model <- paste(dv, "~ treat_14")
ivs_std2 <- c("party_cat2", ivs_std[-(ivs_std %in% c("party_cat"))])
...
var_labels <- list("treat_14" = "Treatment", 
                   "treat_14:party_cat2Independent" = "Treatment x Independent", 
                   "treat_14:party_cat2Republican" = "Treatment x Republican",
                   "treat_14:party_cat2Other" = "Treatment x Other Party",
                   "party_cat2Independent" = "Independent",
                   "party_cat2Republican" = "Republican", 
                   "party_cat2Other" = "Other Party", 
                   "age_std" = "Age (std.)", 
                   "female" = "Female", 
                   "born" = "U.S. born", 
                   "raceWhite" = "White", 
...
m2 <- lm_robust(formula(paste(model, "treat_14 * party_cat2 + ", controls, sep = " + ")), 
                se_type = "stata", fixed_effects = ~ as.factor(year), 
                data = gss_analysis, subset = (treat_14==1|control==1))
...
m2_noschool <- lm_robust(formula(paste(model, "treat_14 * party_cat2", controls, sep = " + ")), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                         subset = (treat_14==1 & school_shooting==0|control==1))
...
m2_school <- lm_robust(formula(paste(model, "treat_14 * party_cat2", controls, sep = " + ")), 
                       se_type = "stata", fixed_effects = ~ as.factor(year), 
                       data = gss_analysis, 
                       subset = (treat_14==1 & school_shooting==1|control==1 & year_school==1))
