# ---
# SUPPLEMENTARY ANALYSIS: REGION FIXED EFFECTS
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_region_FE/"
...
m1 <- lm_robust(formula(paste(model, controls, sep = " + ")), 
                  se_type = "stata", fixed_effects =  ~ as.factor(year) + as.factor(region), 
...
screenreg(list(m1, m2),
          ...
          custom.gof.rows = 
            list("Num. Groups (Year)" = c(length(m1$felevels[[1]]), length(m2$felevels[[1]])), 
                 "Num. Groups (Region)"  = c(length(m1$felevels[[2]]), length(m2$felevels[[2]]))))
...
screenreg(list(m1_noschool, m2_noschool, m1_school, m2_school),
          ...
          custom.gof.rows = 
            list("Num. Groups (Year)" = c(length(m1_noschool$felevels[[1]]), length(m2_noschool$felevels[[1]]), 
                                          length(m1_school$felevels[[1]]), length(m2_school$felevels[[1]])),
                 "Num. Groups (Region)"  = c(length(m1_noschool$felevels[[2]]), length(m2_noschool$felevels[[2]]), 
                                             length(m1_school$felevels[[2]]), length(m2_school$felevels[[2]]))))
