# ---
# SUPPLEMENTARY ANALYSIS: Seasonality (Monthly dummies)
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_month/"
...
var_labels <- list("treat_14" = "Treatment", 
                   ...
                   "as.factor(month)2"  = "February",
                   "as.factor(month)3"  = "March",
                   "as.factor(month)4"  =  "April",
                   "as.factor(month)5"  =    "May",
                   "as.factor(month)6" =    "June",
                   "as.factor(month)7"  =  "July",
                   "as.factor(month)8"  =   "August",
                   "as.factor(month)9"  =  "September",
                   "as.factor(month)10" =  "October",
                   "as.factor(month)11" =   "November",
                   "as.factor(month)12" =  "December"
                   )

# ANALYSIS ------------------------------------------------------------------------------------

m1 <- lm_robust(formula(paste(model, controls, "as.factor(month)", sep = " + ")), 
                se_type = "stata", fixed_effects = ~ as.factor(year), 
                data = gss_analysis, subset = (treat_14==1|control==1))

m2 <- lm_robust(formula(paste(model, "treat_14 * party_cat + ", controls, "as.factor(month)", sep = " + ")), 
                se_type = "stata", fixed_effects = ~ as.factor(year), 
                data = gss_analysis, subset = (treat_14==1|control==1))

# BY SCHOOL SHOOTING ------------------------
m1_noschool <- lm_robust(formula(paste(model, controls, "as.factor(month)", sep = " + ")), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                         subset = (treat_14==1 & school_shooting==0|control==1))


m2_noschool <- lm_robust(formula(paste(model, "treat_14 * party_cat", controls, "as.factor(month)", sep = " + ")), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                         subset = (treat_14==1 & school_shooting==0|control==1))

m1_school <- lm_robust(formula(paste(model, controls, "as.factor(month)", sep = " + ")), 
                       se_type = "stata", fixed_effects = ~ as.factor(year), 
                       data = gss_analysis,
                       subset = (treat_14==1 & school_shooting==1|control==1 & year_school==1))

m2_school <- lm_robust(formula(paste(model, "treat_14 * party_cat", controls, "as.factor(month)", sep = " + ")), 
                       se_type = "stata", fixed_effects = ~ as.factor(year), 
                       data = gss_analysis
                       , subset = (treat_14==1 & school_shooting==1|control==1 & year_school==1))
