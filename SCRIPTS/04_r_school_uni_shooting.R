# ---
# SUPPLEMENTARY ANALYSIS: SCHOOL AND UNI SHOOTINGS
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_school_uni_shooting/"
...
m1_noschool <- lm_robust(formula(paste(model, controls, sep = " + ")), 
                           ...
                           data = gss_analysis, 
                         subset = (treat_14==1 & school_uni_shooting==0|control==1))
...
m1_school <- lm_robust(formula(paste(model, controls, sep = " + ")), 
                         se_type = "stata", fixed_effects = ~ as.factor(year), 
                         data = gss_analysis, 
                       subset = (treat_14==1 & school_uni_shooting==1|control==1 & year_school_uni==1))
...
# combine models 
full_models <- rbind(add_complete_treat_ft(m1_noschool, model = "M1", treat_var = "treat_14", 
                                           specs = "School & College Shooting", sample = "No school"), 
                     add_complete_treat_ft(m2_noschool, model = "M2", treat_var = "treat_14", 
                                           specs = "School & College Shooting", sample = "No school"), 
                     add_complete_treat_ft(m1_school, model = "M1", treat_var = "treat_14", 
                                           specs = "School & College Shooting", sample = "School"), 
