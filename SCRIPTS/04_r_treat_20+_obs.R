# ---
# SUPPLEMENTARY ANALYSIS: RESTRICT TO TREATED CASES WITH AT LEAST 20 OBS
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_treat_20+_obs/"
...
# restrict analysis to at least 20 obs
gss_analysis <- gss_analysis %>%
  filter(!is.na(treat_control_14)) %>%
  group_by(shooting_id, treat_control_14) %>%
  mutate(n_treat = n()) %>%
  filter((treat_control_14=="Treated" & n_treat>=20)|treat_control_14=="Control") %>%
  ungroup()
