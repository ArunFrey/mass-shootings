# ---
# SUPPLEMENTARY ANALYSIS: Shootings with 10+ fatalities only
# ---
...
tables_path <- "OUTPUT/TABLES/supp/r_10+_fatalities/"
...
gss_analysis <- gss_event %>% 
  filter(sample_ivs == 1) %>% 
  # restrict to shootings with 10 or more fatalities
  filter(is.na(fatalities)|fatalities>=10) 

# Only select years where shooting with 10+ fatalities took place
treat_years <- unique(gss_analysis$year[gss_analysis$treat_14==1])
